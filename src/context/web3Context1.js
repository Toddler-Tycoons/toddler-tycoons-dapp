...
const web3Modal = new Web3Modal({
    network: {
        chainId: 80001,
        nodeUrl: "https://speedy-nodes-nyc.moralis.io/8446a7e044dc4043c63d04d8/polygon/mumbai",
    },
    cacheProvider: true,
    providerOptions,
});
...
const Web3ContextProvider = props => {
    ...
    const [web3, setWeb3] = useState(new Web3("https://speedy-nodes-nyc.moralis.io/8446a7e044dc4043c63d04d8/polygon/mumbai"));
    const [tUSDCContract, setTUSDCContract] = useState(new web3.eth.Contract(tUSDCAbi, tUSDCAddress));
    const [mintContract, setMintContract] = useState(new web3.eth.Contract(mintAbi, mintAddress));
    ...
    const mint = async _qty => {
        return await mintContract.methods.mintNFT(account, _qty).send({ from: account });
    };

    const approve = async () => {
        return await tUSDCContract.methods.approve(mintAddress, "1000000000000000000000000").send({ from: account });
    };
